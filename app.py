...
st.set_page_config(page_title="Turnera Completa Final", layout="wide")
...
st.subheader("📆 Vista semanal (semana actual)")
hoy = datetime.today()
...
for hora in horas:
    ...
    for i, dia in enumerate(dias):
        ...
        if not match.empty:
            ...
            if cols[i + 1].button(f"{paciente}", key=f"actual_{hora}_{dia}"):
...
st.subheader("📆 Vista semanal (semana siguiente)")
lunes_siguiente = lunes_actual + timedelta(days=7)
df_siguiente = generar_turnos_disponibles(lunes_siguiente)

horas_s = sorted(df_siguiente["Hora"].unique())
dias_s = sorted(df_siguiente["Fecha"].unique())

for hora in horas_s:
    cols = st.columns(len(dias_s) + 1)
    cols[0].markdown(f"**{hora}**")
    for i, dia in enumerate(dias_s):
        ...
        if not match.empty:
            paciente = match.iloc[0]["Paciente"]
            turno_id = match.iloc[0]["ID"]
            if cols[i + 1].button(f"{paciente}", key=f"siguiente_{hora}_{dia}"):
                st.session_state["turno_seleccionado"] = {
                    "id": turno_id,
                    "paciente": paciente,
                    "fecha": dia,
